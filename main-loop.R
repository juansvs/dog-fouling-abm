...
hist(exp_inv, probability = TRUE, ylim = c(0, lambda),
     main = "", xlab = "t", ylab = "density")
lines(x, dexp(x, rate = lambda))

# Weibull calculation
w <- rweibull(n = 1000, shape = 1.5, scale = 2)

hist(w, probability = TRUE, ylim = c(0, 1.5 / 2),
     main = "", xlab = "t", ylab = "density")
...
# create a simple set of entrances (on edge):
# 4 entrances roughly in middle of each side
entrances <- tibble(
  id = 1:4,
...
path_cells <- bind_rows(path_cells,
  tibble(x = pmin(pmax(wx, 1), GRID_SIZE),
         y = pmin(pmax(wy, 1), GRID_SIZE))
)

path_cells <- distinct(path_cells)

# Plot out the park
plot(path_cells$x, path_cells$y,
  type = "l", asp = 1, xlab = "x", ylab = "y",
  main = "Park path", col = "blue", lwd = 2
)

plot(path_cells$x, path_cells$y, pch = 16, cex = 0.3) # individual points
...
for (day in 1:SIM_DAYS) {
  ...
  for (i in seq_len(nrow(dogs))) {
    ...
    weight_surf <- fouling_weight_surface(d$entry_x, d$entry_y,
                                          on_lead = d$on_lead)
  ...
  if (nrow(faeces) > 0) {
    faeces <- mutate(faeces, age = age + 1L)
